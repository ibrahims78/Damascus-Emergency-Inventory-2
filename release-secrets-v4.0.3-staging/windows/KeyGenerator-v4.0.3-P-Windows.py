...
LOG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "issued-licenses-v4.0.3-windows.log")
...
def log_issued(device_id, license_id, expires_at):
    ...
    line = f"[{now}]  windows  |  Device: {device_id:<40}  License: {license_id}  Expires: {expires_at or 'never'}\n"
...
def print_header():
    w = 67
    print("=" * w)
    print("  Damascus Health Directorate  -  License Key Generator  v4.0.3-P  (windows)")
    print("  Signer keyId: 769e2865cd0d57f3")
    print("=" * w)
    print("  Admin tool — generates activation licenses for windows devices")
...
def main():
    ...
    print_header()
    if _SEED is None:
        print("  [!] Signing key not found (license-private-key.pem).")
        print("      Run this generator from release-secrets/windows, or place the key")
        print("      beside the script and try again.")
        print()
