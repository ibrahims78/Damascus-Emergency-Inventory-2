...
- بناء أندرويد الكامل يحتاج Android Studio + JDK 17 (`android/build-android-apk.ps1`) — البديل المختصر: `releases/v4.0.0/android/build-android-apk.ps1` (طريقة ترقيع v3.0.1 + apksig، تعمل بلا Studio عبر `%LOCALAPPDATA%\dme-toolchain`)
- بناء حزم سطح المكتب: `releases/v4.0.0/scripts/reassemble-electron.ps1` (يستخدم قالب `main-template.cjs` وينسخ `lib/db/desktop-schema.sql`)
...
./releases/v4.0.0/scripts/reassemble-electron.ps1
```
...
| `releases/v4.0.0/scripts/main-template.cjs` | قالب إقلاع إلكترون (المنافذ الثابتة + تمرير الترخيص) |
...
releases/v4.0.0/  سكربتات التجميع + المولدات + مفاتيح التحقق العامة
