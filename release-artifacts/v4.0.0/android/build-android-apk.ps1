...
# Requires: Android Studio (or JDK 17 + Android SDK) on the machine.
# Run from the project root on a machine WITH the Android toolchain:
#   powershell -ExecutionPolicy Bypass -File release-artifacts/v4.0.0/android/build-android-apk.ps1
...
$env:RELEASE_KEYSTORE_PATH = Join-Path $root 'release-artifacts\v4.0.0\release-secrets\android\android-release.keystore'
# STEP (one time): generate the keystore on the build machine with keytool:
#   keytool -genkeypair -v -keystore release-artifacts/v4.0.0/release-secrets/android/android-release.keystore -alias dme -keyalg RSA -keysize 2048 -validity 10000
